...
ITEMS = {
    "gold_necklace": {"title": "Ожерелье", "emoji_name": "Extravagant_salvaged_necklace", "price": 34500, "group": "gold"},
    "gold_earring": {"title": "Серьга", "emoji_name": "Extravagant_salvaged_earring", "price": 30000, "group": "gold"},
    "gold_bracelet": {"title": "Браслет", "emoji_name": "Extravagant_salvaged_bracelet", "price": 28500, "group": "gold"},
    "gold_ring": {"title": "Кольцо", "emoji_name": "Extravagant_salvaged_ring", "price": 27000, "group": "gold"},
    "silver_necklace": {"title": "Ожерелье", "emoji_name": "Salvaged_necklace", "price": 13000, "group": "silver"},
    "silver_earring": {"title": "Серьга", "emoji_name": "Salvaged_earring", "price": 10000, "group": "silver"},
    "silver_bracelet": {"title": "Браслет", "emoji_name": "Salvaged_bracelet", "price": 9000, "group": "silver"},
    "silver_ring": {"title": "Кольцо", "emoji_name": "Salvaged_ring", "price": 8000, "group": "silver"},
}
...
def normalize_fleet_state(data):
    ...
    if isinstance(data, dict):
        ...
        if isinstance(old_fcs, dict):
            for fc_key, cfg in FC_CONFIG.items():
                old_fc = old_fcs.get(fc_key, {})
                if isinstance(old_fc, dict):
                    state["fcs"][fc_key]["active_subs"] = safe_int(
                        old_fc.get("active_subs", cfg["active_subs"]),
                        cfg["active_subs"],
                    )
                    state["fcs"][fc_key]["active_subs"] = max(
                        0,
                        min(4, state["fcs"][fc_key]["active_subs"]),
                    )
                    ...
                    if isinstance(old_subs, list):
                        for i in range(min(4, len(old_subs))):
                            ...
                            if not isinstance(old_sub, dict):
                                continue
                            state["fcs"][fc_key]["subs"][i]["return_time"] = old_sub.get("return_time")
                            state["fcs"][fc_key]["subs"][i]["ready_notified"] = bool(old_sub.get("ready_notified", False))
...
def parse_image(image_bytes):
    ...
    for line in voyage_lines:
        ...
        result.append(now_utc() + timedelta(minutes=total_minutes))
...
def get_next_returns(state, limit=5):
    items = []
    for fc_key, cfg in FC_CONFIG.items():
        active = get_fc_active_count(state, fc_key)
        for idx, sub in enumerate(state["fcs"][fc_key]["subs"][:active], 1):
            ...
            if mins is None or mins <= 0:
                continue
            items.append((rt, f"{cfg['short']}-{idx}"))

    items.sort(key=lambda x: x[0])
    lines = []
    for rt, label in items[:limit]:
        left, _ = format_remaining(rt)
...
def get_custom_emoji(guild, emoji_name):
    ...
    return str(found) if found else "▫️"


def item_total(data, item_id):
    return int(data["inventory"].get(item_id, 0)) * int(ITEMS[item_id]["price"])


def group_total(data, group):
    return sum(item_total(data, item_id) for item_id, item in ITEMS.items() if item["group"] == group)


def treasury_total():
    data = load_treasury()
    return group_total(data, "gold") + group_total(data, "silver")
...
def get_treasury_status():
    ...
    local_time = to_andryukha_time(updated_dt).strftime("%d.%m • %H:%M")
...
def item_line(guild, data, item_id):
    item = ITEMS[item_id]
    emoji = get_custom_emoji(guild, item["emoji_name"])
    count = int(data["inventory"].get(item_id, 0))
    total = item_total(data, item_id)
    return f"{emoji} **{item['title']}** — **{count}** шт. • **{format_gil(total)}**"


def build_treasury_embed(guild):
    data = load_treasury()
    gold_lines = [item_line(guild, data, f"gold_{kind}") for kind in ITEM_ORDER]
    silver_lines = [item_line(guild, data, f"silver_{kind}") for kind in ITEM_ORDER]
    ...
    embed.add_field(name="🏅 Золото", value="\n".join(gold_lines) + f"\n\n**Итого золото:** `{format_gil(gold_total)}`", inline=False)
    embed.add_field(name="🥈 Серебро", value="\n".join(silver_lines) + f"\n\n**Итого серебро:** `{format_gil(silver_total)}`", inline=False)
    embed.add_field(name="💰 Общая сумма", value=f"**{format_gil(total)} gil**", inline=False)
    embed.add_field(name=status_text, value=f"🕒 Последнее обновление: `{updated_text}`", inline=False)
    return embed
...
def build_fc_field(state, fc_key):
    ...
    for idx, sub in enumerate(fc["subs"][:active], 1):
        ...
        if rt is None:
            lines.append(f"`{idx}` {dot} {sub['name']}\n`   —          —     —`")
            continue
...
def build_dashboard_embed():
    ...
    next_returns = get_next_returns(state, limit=5)
    ...
    embed.add_field(
        name="🕒 Ближайшие",
        value="\n".join(next_returns) if next_returns else "—",
        inline=True,
    )
    embed.add_field(
        name="💰 Казна / склад",
        value=f"`{format_gil(total)} gil`\n{treasury_status_text}\n`{treasury_updated_text}`",
        inline=True,
    )
    ...
    for fc_key, cfg in FC_CONFIG.items():
        ...
        if fc_key == "eva":
            title += " (Основная)"
        title += f" • {active}/4"
        embed.add_field(name=title, value=build_fc_field(state, fc_key), inline=True)

    embed.set_footer(text="🟢 готово • 🔵 0–6ч • 🟡 6–12ч • 🟠 12–24ч • 🔴 >24ч | Андрюха UTC+5 • Валера UTC+7")
    return embed
...
async def find_timer_dashboard_message(guild=None):
    ...
    if channel_id and message_id:
        ...
        if channel is not None:
            try:
                dashboard_message = await channel.fetch_message(int(message_id))
                return dashboard_message
            except Exception:
                dashboard_message = None
    ...
    try:
        async for msg in timer_channel.history(limit=50):
            if msg.author.id != client.user.id or not msg.embeds:
                continue
            if msg.embeds[0].title and "SUBMARINE COMMAND CENTER" in msg.embeds[0].title:
...
async def refresh_dashboard(channel=None, guild=None):
    ...
    dashboard_message = await channel.send(embed=embed, view=view)
    state = load_fleet_state()
...
async def refresh_treasury_message(interaction=None, channel=None):
    ...
    embed = build_treasury_embed(target_channel.guild)
    view = TreasuryView()
    message = await find_existing_treasury_message(target_channel)
    ...
    def __init__(self):
        ...
        for fc_key, cfg in FC_CONFIG.items():
            ...
            button = ui.Button(
                ...
                row=0 if list(FC_CONFIG).index(fc_key) < 4 else 1,
                disabled=active <= 0,
            )
            button.callback = self.make_select_callback(fc_key)
    ...
    def make_select_callback(self, fc_key):
        async def callback(interaction):
            pending_scans[interaction.user.id] = {
                "fc_key": fc_key,
                "expires_at": now_utc() + timedelta(seconds=SCAN_TIMEOUT_SECONDS),
            }
            cfg = FC_CONFIG[fc_key]
            await interaction.response.send_message(
                f"{cfg['emoji']} Жду скрин для **{cfg['title']}** {SCAN_TIMEOUT_SECONDS} сек.",
                ephemeral=True,
                delete_after=10,
            )
        return callback
...
class GoldModal(ui.Modal):
    def __init__(self):
        super().__init__(title="🏅 Обновить золото")
        inventory = load_treasury()["inventory"]
        self.necklace = ui.TextInput(label="Ожерелье", default=str(inventory.get("gold_necklace", 0)), required=True, max_length=4)
        self.earring = ui.TextInput(label="Серьга", default=str(inventory.get("gold_earring", 0)), required=True, max_length=4)
        self.bracelet = ui.TextInput(label="Браслет", default=str(inventory.get("gold_bracelet", 0)), required=True, max_length=4)
        self.ring = ui.TextInput(label="Кольцо", default=str(inventory.get("gold_ring", 0)), required=True, max_length=4)
    ...
    async def on_submit(self, interaction):
        ...
        try:
            ...
            await interaction.response.send_message("Нужно вводить только числа.", ephemeral=True, delete_after=10)
            return

        save_treasury(data)
        await interaction.response.defer(ephemeral=True)
        await refresh_treasury_message(interaction=interaction)
        await refresh_dashboard(guild=interaction.guild)


class SilverModal(ui.Modal):
    def __init__(self):
        super().__init__(title="🥈 Обновить серебро")
        inventory = load_treasury()["inventory"]
        self.necklace = ui.TextInput(label="Ожерелье", default=str(inventory.get("silver_necklace", 0)), required=True, max_length=4)
        self.earring = ui.TextInput(label="Серьга", default=str(inventory.get("silver_earring", 0)), required=True, max_length=4)
        self.bracelet = ui.TextInput(label="Браслет", default=str(inventory.get("silver_bracelet", 0)), required=True, max_length=4)
        self.ring = ui.TextInput(label="Кольцо", default=str(inventory.get("silver_ring", 0)), required=True, max_length=4)
        self.add_item(self.necklace)
        self.add_item(self.earring)
        self.add_item(self.bracelet)
        self.add_item(self.ring)
...
class TreasuryView(ui.View):
    ...
    @ui.button(label="Золото", emoji="🏅", style=discord.ButtonStyle.primary, custom_id="treasury_gold")
    async def gold_button(self, interaction, button):
        await interaction.response.send_modal(GoldModal())

    @ui.button(label="Серебро", emoji="🥈", style=discord.ButtonStyle.primary, custom_id="treasury_silver")
    async def silver_button(self, interaction, button):
        await interaction.response.send_modal(SilverModal())


async def send_ready_alert(channel, fc_key, sub_index, sub_name, rt):
    cfg = FC_CONFIG[fc_key]
    embed = discord.Embed(
        title=f"🚨 {cfg['emoji']} {cfg['title']} • {sub_name} ГОТОВА",
        color=0x2ECC71,
    )
    embed.add_field(name="Андрюха", value=to_andryukha_time(rt).strftime("%H:%M"), inline=True)
    embed.add_field(name="Валера", value=to_valera_time(rt).strftime("%H:%M"), inline=True)
    embed.add_field(name="Статус", value="💰 +БАБКИ, ЗАПУСКАЙ ПО НОВОЙ 💰", inline=False)

    msg = await channel.send(get_ping_text(), embed=embed)
    await refresh_dashboard(channel=channel, guild=channel.guild)
        ...
        try:
            ...
            if channel is not None:
                for fc_key, cfg in FC_CONFIG.items():
                    ...
                    for idx, sub in enumerate(state["fcs"][fc_key]["subs"][:active], 1):
                        ...
                        if rt is None:
                            continue
                        if not sub.get("ready_notified") and now_utc() >= rt:
                            sub["ready_notified"] = True
                            changed = True
                            asyncio.create_task(send_ready_alert(channel, fc_key, idx, sub["name"], rt))
            ...
            try:
                ...
                if treasury_channel_id:
                    treasury_channel = client.get_channel(int(treasury_channel_id))
                    if treasury_channel is None:
                        try:
                            treasury_channel = await client.fetch_channel(int(treasury_channel_id))
                        except Exception:
                            treasury_channel = None
                    if treasury_channel is not None:
                        await refresh_treasury_message(channel=treasury_channel)
            except Exception as e:
...
async def handle_scan_image(message, fc_key):
    ...
    try:
        ...
        fc = state["fcs"][fc_key]
        for idx, rt in enumerate(new_times[:active]):
            fc["subs"][idx]["return_time"] = rt.isoformat()
            fc["subs"][idx]["ready_notified"] = False

        save_fleet_state(state)
        await refresh_dashboard(channel=message.channel, guild=message.guild)
...
async def on_ready():
    ...
    client.add_view(FleetSelectView())
    client.add_view(TreasuryView())
    print("Bot ready")
...
async def on_message(message):
    ...
    if low in ["!dashboard", "!dash", "!таймер", "!timer", "!subs"]:
        await refresh_dashboard(channel=message.channel, guild=message.guild)
        try:
            await message.delete()
        except Exception:
    ...
    if low in ["!treasury", "!склад", "!fc"]:
        await refresh_treasury_message(channel=message.channel)
        try:
            await message.delete()
        except Exception:
            pass
    ...
    if low.startswith("!scan ") or low.startswith("!скан "):
        raw_fc = content.split(maxsplit=1)[1]
        fc_key = resolve_fc_key(raw_fc)
        if fc_key is None:
            await message.add_reaction("❌")
            return
    ...
    if low.startswith("!slots ") or low.startswith("!лодки "):
        parts = content.split()
        if len(parts) != 3:
            await message.add_reaction("❌")
            return

        fc_key = resolve_fc_key(parts[1])
        count = safe_int(parts[2], -1)
        if fc_key is None or count < 0 or count > 4:
            await message.add_reaction("❌")
            return
    ...
    if message.attachments:
        pending = pending_scans.get(message.author.id)
        if pending and now_utc() <= pending["expires_at"]:
            fc_key = pending["fc_key"]
            handled = await handle_scan_image(message, fc_key)
            if handled:
                pending_scans.pop(message.author.id, None)
                return
        elif pending:
            pending_scans.pop(message.author.id, None)
